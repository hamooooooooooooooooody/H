...
distube.on("addSong", (queue, song) => {
	try {
		queue.textChannel.send({ content: `**${config.music}  ${song.name} - \`(${song.formattedDuration})\` **to the queue by ${song.user}` });
	} catch (err) {
		console.log(err);
	}
});
...
function receiveQueueData(newQueue, newTrack) {
	try {
		...
		var embed = new EmbedBuilder()
			...
			.setFooter({
				text: `Added by ${newTrack.user.username}  |  Volume: ${newQueue.volume}%  |  Queue length: ${newQueue.songs.length - 1} | BY : Dealer Lounge`,
